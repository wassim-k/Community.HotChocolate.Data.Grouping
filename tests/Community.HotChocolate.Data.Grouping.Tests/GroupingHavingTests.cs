...
public class GroupingHavingTests(GroupingTestFixture fixture) : IClassFixture<GroupingTestFixture>
{
    ...
    public static TheoryData<string, string, IReadOnlyDictionary<string, object?>?> Cases() => new()
    {
        ...
        {
            // Bonus is decimal?, so some employees have it, some don't. `eq: null` must
        ...
        {
            // `nin` against a list must NOT match null aggregate slots, matching SQL's
        ...
        },
        {
            ...
            // Acme: count=4 pass, sum(salary)=360000 pass; Globex: count=4 pass, sum(salary)=345000 fail.
        ...
        },
        {
            ...
            // types. Acme min(salary)=60000 (Grace) passes; Globex min=75000 (Frank) fails.
            "MinHaving",
            """
            query EmployeeGrouping {
                employeeGrouping {
        ...
        {
            // HAVING on Max with the inclusive gte variant. Acme max=120000 passes; Globex max=95000 fails.
            "MaxHavingGte",
            """
            query EmployeeGrouping {
                employeeGrouping {
        ...
        },
        {
            ...
            // Acme min(name)="Alice" passes; Globex min(name)="Dave" fails.
        ...
        {
            // Positive `in: [...]` (sibling of NinNullSafe). Acme sum(bonus)=30000 passes;
            // Globex sum(bonus)=15500 fails.
            "InPositive",
            """
            query EmployeeGrouping {
                employeeGrouping {
        ...
        {
            // A having variable that's omitted (optional, unset) must mean "no constraint", so every
