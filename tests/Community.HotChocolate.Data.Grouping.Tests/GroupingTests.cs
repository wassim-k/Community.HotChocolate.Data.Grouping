...
public partial class GroupingTests(GroupingTestFixture fixture) : IClassFixture<GroupingTestFixture>
{
    ...
    public static TheoryData<string, string> MiddlewareTest_Data => new()
    {
        ...
        {
            // No `key` selected, so the entire source collapses into a single bucket and
        ...
        },
        {
            ...
            // the SelectMany prefix, so it rebases to nothing and the filter is a no-op
        ...
        },
        {
            ...
            // contributes |Projects| x |Skills| rows after the chained SelectMany.
