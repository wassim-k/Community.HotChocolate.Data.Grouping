...
public class GroupingConventionTests
{
    ...
    [Fact]
    public async Task AllowedAggregations_None_OmitsAggregateFieldAndTypes()
    {
        var services = new ServiceCollection();
        ...
        Assert.Contains(grouping.Fields, f => f.Name == "key");
        Assert.Contains(grouping.Fields, f => f.Name == "count");
        Assert.DoesNotContain(grouping.Fields, f => f.Name == "aggregate");

        Assert.False(schema.Types.ContainsName("ProductAggregate"));
        Assert.False(schema.Types.ContainsName("IntAggregateResult"));
    }
    ...
    public sealed class MoneyAggregateResultType : AggregateResultType
    {
        protected override void Configure(IAggregateResultTypeDescriptor descriptor)
        {
            ...
            descriptor.Operation(GroupingAggregations.Min).Type(typeof(Money?)).Having<Money>();
            descriptor.Operation(GroupingAggregations.Max).Type(typeof(Money?)).Having<Money>();
        }
    }
    ...
    {
        // Three seeded products produce one bucket with min "5 USD", max "25 USD".
        var services = new ServiceCollection();
        services
            .AddGraphQL()
            .AddFiltering(f => f.AddDefaults().BindRuntimeType<Money, MoneyOperationFilterInputType>())
    ...
    public async Task BindRuntimeType_OrEnabledIntFilter_FlowsToHaving()
    {
        ...
        // Seed produces one bucket with count=3, so OR with a matching arm keeps it, OR with
    ...
    public async Task Operation_Rename_ReshapesSchemaAndDispatch()
    {
        ...
        // input and runtime dispatch, with no grouping-side operation table.
        var services = new ServiceCollection();
        services
            .AddGraphQL()
            .AddFiltering(f =>
    ...
    public async Task CustomOperator_DivisibleBy_RegistersAndDispatches()
    {
        ...
        // Seed produces one bucket with count=2, so `divisibleBy: 2` matches, `divisibleBy: 3` doesn't.
        var services = new ServiceCollection();
        services
            .AddGraphQL()
            .AddFiltering(f =>
