...
- **A collection in `aggregate` requires the same collection in `key`.** Otherwise the join would happen silently, drop entities without that collection, and turn `count` into a flattened-row count without the user asking for it. Rejected at parse time with `GROUPING_AGGREGATE_COLLECTION_MISSING_FROM_KEY`. See [Aggregating across a collection key](../examples/aggregations#aggregating-across-a-collection-key).
- **No correlated-subquery magic.** Some engines (notably Hasura) emit per-relationship subqueries, which keeps sibling aggregates independent. We deliberately don't; it forces provider-specific knowledge. The cost is honest: sibling collections inflate each other under JOIN semantics, exactly as SQL would. See [Sibling collections multiply the fan-out](./runtime-quirks#sibling-collections-multiply-the-fan-out) for the worked example.
...
- **One `*AggregateResult` per scalar, one `*Aggregate` per entity.** The per-entity `*Aggregate` is reused at every nesting level, so adding an entity adds no aggregate-result types.
...
`having` is a per-operation argument whose input type is HotChocolate.Data's `*OperationFilterInput` for the operation's (widened) result scalar, not a separate per-entity HAVING input. A predicate on `salary.sum` is semantically attached to *that* operation, not to the entity (same motivation as field-first aggregates). And reusing HotChocolate.Data's existing `*OperationFilterInput` types keeps the schema small: there's no parallel `*HavingInput` type per entity, and `having:` arguments just reference the filter inputs the schema already carries. Predicates from multiple `having` arguments AND-combine. Filter-side concerns (operators, handlers, type bindings) go through `AddFiltering`. See [HAVING Filtering](../configuration/having-filtering#having-needs-addfiltering).
