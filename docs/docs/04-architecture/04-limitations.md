...
`or:` **within** a single `having: { ... }` clause is supported on filter inputs that expose it. See [Enabling OR within a HAVING clause](../configuration/having-filtering#enabling-or-within-a-having-clause).
