...
Single-valued navigations (one-to-one, many-to-one) **are** different: their FK column can be `NULL`, which surfaces as a `null` bucket (see [Distinguishing ancestor-null from leaf-null](../examples/grouping#distinguishing-ancestor-null-from-leaf-null)).
...
`eq: null` / `neq: null` emit explicit `IS NULL` / `IS NOT NULL` (not lifted equality, which would diverge between in-memory and EF). `nin: [...]` adds an `agg != null` guard so a null aggregate slot never satisfies `NOT IN`, matching SQL semantics consistently across all three providers.
