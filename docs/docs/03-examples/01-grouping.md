...
A `key` path crossing a collection navigation inserts a `SelectMany` before grouping, so each parent row contributes one row per element (SQL `JOIN` semantics).
...
Aggregates over a collection-keyed group run on flattened rows, not entities. See [Aggregating across a collection key](./aggregations#aggregating-across-a-collection-key).
...
For post-grouping bucket filtering use the per-operation `having` argument. See [Filtering buckets with `having`](./aggregations#filtering-buckets-with-having).
