...
public sealed class GroupingResultBuilder<T>
{
    ...
    /// </remarks>
    public GroupingResultBuilder<T> SetAggregate(PathSegment[] path, AggregationKind kind, object? value)
    {
        var leaf = EnsureLeafSlot(_aggregates, path);
        switch (kind)
        {
            case AggregationKind.Avg: leaf.Avg = value; break;
            case AggregationKind.Sum: leaf.Sum = value; break;
            case AggregationKind.Min: leaf.Min = value; break;
            case AggregationKind.Max: leaf.Max = value; break;
            default: throw ThrowHelper.Grouping_UnknownAggregationKind();
        }
        return this;
    }
