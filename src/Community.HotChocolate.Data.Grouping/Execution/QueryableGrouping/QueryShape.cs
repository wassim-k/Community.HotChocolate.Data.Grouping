...
internal abstract record QueryShape(Expression Source, Type RowType)
{
        ...
        bool inMemory)
    {
        ...
        var prefixes = GroupingHelpers.ResolveCollectionPrefixes(paths);
    ...
    private static Expression ApplyParentFilters(QueryShape shape, PathSegment[][] paths)
    {
        ...
        return Expression.Call(
            _whereMethod.MakeGenericMethod(shape.RowType),
            shape.Source,
    ...
    {
        private static readonly MethodInfo _selectManyMethod = typeof(Queryable).GetMethods().Single(m =>
        ...
        public static SinglePrefix Build(Expression source, Type entityType, CollectionPrefix prefix, bool inMemory)
        {
            ...
            var flattened = Expression.Call(
                _selectManyMethod.MakeGenericMethod(entityType, prefix.ElementType),
                source,
    ...
    {
        private static readonly MethodInfo _selectManyWithResultMethod = typeof(Queryable).GetMethods().Single(m =>
            m.Name == nameof(Queryable.SelectMany)
            ...
            bool inMemory)
        {
            ...
            for (var step = 0; step < prefixes.Count; step++)
            {
                ...
                current = Expression.Call(
                    _selectManyWithResultMethod.MakeGenericMethod(rowType, prefix.ElementType, nextType),
                    current,
