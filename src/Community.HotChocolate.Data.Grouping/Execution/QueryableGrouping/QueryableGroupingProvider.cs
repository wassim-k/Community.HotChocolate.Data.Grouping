...
{
    private static readonly MethodInfo _createQueryMethod = typeof(IQueryProvider).GetMethods()
        .Single(m => m.Name == nameof(IQueryProvider.CreateQuery) && m.IsGenericMethod);
        ...
        IReadOnlyList<HavingPredicate> having)
    {
        ...
        var grouped = Expression.Call(
            QueryableMethods.GroupBy.MakeGenericMethod(shape.RowType, keyProj.Type),
            shape.Source,
            Expression.Quote(keyProj.Selector));

        var projection = Expression.Call(
            QueryableMethods.Select.MakeGenericMethod(rowProj.GroupingType, rowProj.Type),
            grouped,
            Expression.Quote(rowProj.Selector));

        var projected = (IQueryable)_createQueryMethod
            .MakeGenericMethod(rowProj.Type)
            .Invoke(source.Provider, [projection])!;
    ...
    private static IExecutable WrapAsExecutable<T>(IQueryableExecutable<T>? original, IQueryable projected)
    {
        ...
        {
            var withSource = WithSourceMethod<T>.Definition.MakeGenericMethod(rowType);
    ...
    private static class WithSourceMethod<T>
    {
        public static readonly MethodInfo Definition = typeof(IQueryableExecutable<T>).GetMethods()
        ...
        bool inMemory)
    {
        ...
        return (IQueryable)QueryableMethods.Where
