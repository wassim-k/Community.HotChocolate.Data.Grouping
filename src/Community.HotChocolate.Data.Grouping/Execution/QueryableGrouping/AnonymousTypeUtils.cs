...
internal sealed class CarrierSlot
{
    private readonly PropertyInfo _property;
    private readonly Func<object, object?> _getter;

    public CarrierSlot(PropertyInfo property)
    {
        _property = property;
        _getter = BuildGetter(property);
    }

    public Type Type => _property.PropertyType;

    public object? GetValue(object? instance) =>
        instance is null ? null : _getter(instance);

    public Expression Access(Expression target) => Expression.Property(target, _property);
