...
{
    private AggregateResultOperationDescriptor(IDescriptorContext context, AggregationKind kind)
        : base(context)
    {
    ...
    public AggregationKind Kind => Configuration.Kind;
    ...
    private static PureFieldDelegate GetResolver(AggregationKind kind) => kind switch
    {
        AggregationKind.Avg => static ctx => ctx.Parent<AggregateValues>().Avg,
        AggregationKind.Sum => static ctx => ctx.Parent<AggregateValues>().Sum,
        AggregationKind.Min => static ctx => ctx.Parent<AggregateValues>().Min,
        AggregationKind.Max => static ctx => ctx.Parent<AggregateValues>().Max,
        _ => throw new ArgumentOutOfRangeException(nameof(kind), kind, null),
    };

    public static AggregateResultOperationDescriptor New(IDescriptorContext context, AggregationKind kind) =>
        new(context, kind);
}
