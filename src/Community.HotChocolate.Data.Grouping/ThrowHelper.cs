...
internal static class ThrowHelper
{
    ...
    public static InvalidOperationException Grouping_UnknownGroupingAggregations()
    ...
    public static GraphQLException Grouping_AggregateCollectionMissingFromKey(
        ...
        => new GraphQLException(
            ErrorBuilder.New()
                .SetMessage(
                    "Cannot use `{0}` on `{1}` in `aggregate`: it navigates a collection that `key` "
                    + "doesn't group by. Selecting a collection navigation inside `aggregate` expands "
                    + "the source into one row per element, which changes which buckets are returned "
                    + "and what `count` means. Include that collection in `key` so the expansion is "
                    + "visible at the call site, or remove `{1}` from `aggregate`.",
                    aggregateOperation,
                    aggregatePath)
